...
export default function ProductCard ({ item }) {
  ...
  const renderTitle = (title) => title.length > 80 ? title.substr(0, 80).trim() + "..." : title;

  // const renderDesc = (desc) => desc.length > 100 ? desc.substr(0, 100).trim() + "..." : desc;

  const renderDesc = (desc) => desc.length > 100 ? `${desc.substr(0, 80).trim()}...` : desc;
  ...
  return (
    ...
    <div className="flex justify-between items-center">
      ...
      <button className="bg-gray-800 text-white px-2 rounded py-1">Buy Now</button>
    </div>
  </div>);
}
